...
type TemplateData = Prisma.TB_M_TEMPLATECreateInput & {
  NEW_ITEM_CODE?: string;
  NEW_LOCALE?: string;
  NEW_CHANNEL?: string;
};
type TemplateWhereInput = Prisma.TB_M_TEMPLATEWhereInput;

export const templateService = {
  async getTemplate(app: FastifyInstance, query: any) {
    const {
      page = 1,
      limit = 10,
      q,
      ...
      channel,
      locale,
      sortBy = "ITEM_CODE", // Default sort by part of PK
      order = "asc",
    } = query;
    ...
    const orderBy: Prisma.TB_M_TEMPLATEOrderByWithRelationInput = {
      [sortBy]: order,
    };
  ...
  async createTemplate(app: FastifyInstance, data: TemplateData) {
    try {
  ...
  async updateTemplate(app: FastifyInstance, data: TemplateData) {
    ...
    try {
      ...
      if (e instanceof Prisma.PrismaClientKnownRequestError) {
        if (e.code === "P2025") {
          app.log.error(
            `Record not found with key: ${ITEM_CODE}, ${LOCALE}, ${CHANNEL}`
          );
          throw new ApplicationError(
  ...
  async deleteTemplate(
    app: FastifyInstance,
    compoundId: { ITEM_CODE: string; LOCALE: string; CHANNEL: string }
  ) {
    try {
