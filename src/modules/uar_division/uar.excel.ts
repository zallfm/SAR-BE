...
export type UarTemplateParams = {
    systemName: string;        // contoh: "IPPCS"
    divisionName: string;      // contoh: "ISTD"
    departmentName: string;    // contoh: "CIO"
    monthLabel: string;        // contoh: "August 2025"
    roleColumns: string[];     // contoh: ["ROLE A", "ROLE B", "ROLE C"]
};

export async function buildUarExcelTemplate(params: UarTemplateParams) {
    ...
    const fillYellow: FillPattern = {
        type: "pattern",
        pattern: "solid",
        fgColor: { argb: "FFEB00" }, // kuning
    };

    const fillBlue: FillPattern = {
        type: "pattern",
        pattern: "solid",
    ...
    ws.columns = allCols as any; // ExcelJS types longgar untuk columns; aman di-cast
    ...
    for (let c = 1; c <= remarkIndex; c++) {
        ...
        cellTop.alignment = center as Alignment;
        cellTop.font = { bold: true };
        cellTop.border = borderThin;
        if (c >= accessStart && c <= accessEnd || c === remarkIndex) cellTop.fill = fillBlue
        else cellTop.fill = fillYellow;
    }
    for (let c = 1; c <= remarkIndex; c++) {
        ...
        if (c >= accessStart && c <= accessEnd || c === remarkIndex) cell.fill = fillBlue
        else cell.fill = fillYellow;
    }
    ...
    for (let r = 0; r < emptyRows; r++) {
        ...
        for (let c = 1; c <= remarkIndex; c++) {
            ...
            cell.border = borderThin;
            cell.alignment = (c === 4 ? left : center) as Alignment; // EMPLOYEE NAME kiri, lainnya center
        }
        row.height = 18;
    }
