...
import fs from "fs";
const toGB = (d) => d.toLocaleString("en-GB", { hour12: false }).replace(",", "");
const normalizeStatus = (s) => {
    ...
    if (v === "success")
        return "Success";
    if (v === "failure" || v === "error")
        return "Error";
    if (v === "warning")
        return "Warning";
    if (v === "inprogress" || v === "in progress")
        return "InProgress";
    // fallback aman
    return "Success";
};
...
export const logMonitoringController = {
    ...
    listDetails: (app) => async (req, reply) => {
        const result = await logMonitoringService.listDetails(req.params.processId, req.query.page, req.query.limit);
        return reply.status(200).send(result);
    },
    ...
    exportExcel: (app) => async (req, reply) => {
        ...
        const { data, meta } = await logMonitoringService.listLogs(effective);
