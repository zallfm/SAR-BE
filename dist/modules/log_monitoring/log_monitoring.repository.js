...
async function resolveMessageForStatus(status) {
    ...
    if (!msg)
        throw new Error(`TB_M_MESSAGE not found for MESSAGE_TYPE=${wantedType}`);
...
async function allocateProcessId(tx, baseDate) {
    ...
    await tx.$executeRawUnsafe(`EXEC sp_getapplock @Resource = @p1, @LockMode = 'Exclusive', @LockOwner='Transaction', @LockTimeout = 5000;`, resource);
...
function withinRange(dt, start, end) {
    ...
    if (start && t < parseDate(start).getTime())
        return false;
    if (end && t > parseDate(end).getTime())
        return false;
    return true;
}
const toGB = (d) => {
...
export const logRepository = {
    async listLogs(params) {
        const { page = 1, limit = 10, status, module, userId, q, startDate, endDate, sortBy = "START_DATE", order = "desc", } = params;
        const where = {};
        if (status) {
            const map = {
                Success: "S",
        ...
        if (startDate || endDate) {
            where.START_DT = {};
            if (startDate)
                where.START_DT.gte = parseDate(startDate);
            if (endDate)
                where.START_DT.lte = parseDate(endDate);
        }
        if (q) {
            const s = q;
            where.OR = [
    ...
    async insertLog(newLog) {
        ...
        if (!moduleRow)
            throw new Error(`MODULE not found: ${newLog.MODULE}`);
