...
export const runtimeParamService = {
    async getRuntimeParam(app, query) {
        const { page = 1, limit = 10, q, key, sortBy = "KEY", order = "asc", } = query;
    ...
    async createRuntimeParam(app, data) {
        try {
    ...
    async updateRuntimeParam(app, data) {
        ...
        try {
            const updatedParam = await app.prisma.tB_M_APPLICATION_RUNTIME_PARAM.update({
                where: {
                    KEY: KEY,
                },
                data: dataForUpdate,
            ...
            if (e instanceof Prisma.PrismaClientKnownRequestError) {
                ...
                if (e.code === "P2002") {
                    app.log.error(`Unique constraint violation. The new key '${NEW_KEY}' may already exist.`);
                    throw new ApplicationError(ERROR_CODES.APP_ALREADY_EXISTS, ERROR_MESSAGES[ERROR_CODES.APP_ALREADY_EXISTS]);
                }
            }
    ...
    async deleteRuntimeParam(app, key) {
        try {
            const deletedParam = await app.prisma.tB_M_APPLICATION_RUNTIME_PARAM.delete({
                where: {
                    KEY: key,
                },
            });
