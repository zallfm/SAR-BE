export const userService = {
    async getRecipientEmail(app, itemCode, uarId, subjectNoreg) {
        if (itemCode.startsWith("PIC_")) {
            try {
                ...
                if (!uarTask || !uarTask.DIVISION_ID) {
                    app.log.warn(`Could not find UAR task or DIVISION_ID for UAR_ID: ${uarId}, NOREG: ${subjectNoreg}`);
                    return null;
                }
                ...
                app.log.error(error, `Failed to get PIC UAR email for UAR_ID: ${uarId}`);
                return null;
            }
        }
        if (itemCode.startsWith("UAR_") || itemCode.startsWith("DIV_")) {
            try {
                let recipientNoreg = null;
                if (itemCode.startsWith("UAR_")) {
                    ...
                    if (!uarTask || !uarTask.TB_M_APPLICATION) {
                        app.log.warn(`Could not find UAR task or Application for UAR_ID: ${uarId}`);
                        return null;
                    }
                ...
                if (!recipientNoreg) {
                    app.log.warn(`Could not determine a recipient NOREG for ITEM_CODE: ${itemCode}`);
                    return null;
                }
    ...
    async getEmailFromNoreg(app, noreg) {
        if (!app.prismaSC) {
        ...
        try {
            ...
            if (!user) {
                app.log.warn(`No valid user found in TB_M_EMPLOYEE for NOREG: ${noreg}`);
                return null;
            }
            return user.MAIL;
        }
        catch (error) {
            app.log.error(error, `Failed to get user email from TB_H_EMPLOYEE for NOREG: ${noreg}`);
            return null;
        }
    },
};
