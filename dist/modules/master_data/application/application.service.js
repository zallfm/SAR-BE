...
export const applicationService = {
    ...
    normalizeStatus(input) {
        ...
        if (s === "active" || s === "aktif" || s === "a" || s === "0")
            return "Active";
        if (s === "inactive" || s === "inaktif" || s === "i" || s === "1")
            return "Inactive";
        return undefined;
    },
    async activeList() {
    ...
    async getById(id) {
        ...
        if (!row) {
            throw new ApplicationError(ERROR_CODES.APP_NOT_FOUND, "Application not found", { id }, undefined, 404);
        }
        return row;
    },
    async create(input, auditUser) {
        ...
        if (existingApp) {
            throw new ApplicationError(ERROR_CODES.VAL_DUPLICATE_ENTRY, "APPLICATION_ID already exists", { errors: { APPLICATION_ID: { code: "ALREADY_EXISTS", message: "APPLICATION_ID already exists", value: input.APPLICATION_ID } } }, 
        ...
        if (nameUsed) {
            throw new ApplicationError(ERROR_CODES.VAL_DUPLICATE_ENTRY, "APPLICATION_NAME already exists", { errors: { APPLICATION_NAME: { code: "APPLICATION_NAME already existst", value: input.APPLICATION_NAME } } }, 
        ...
        if (norm)
            input.APPLICATION_STATUS = norm;
        return repo.create(input, auditUser);
    },
    async update(id, updates, auditUser) {
        ...
        if (updates.NOREG_SYSTEM_OWNER) {
            ...
            if (updates.APPLICATION_NAME) {
                ...
                if (nameUsed) {
                    throw new ApplicationError(ERROR_CODES.VAL_DUPLICATE_ENTRY, "APPLICATION_NAME already exists", { errors: { APPLICATION_NAME: { code: "APPLICATION_NAME already existst", value: updates.APPLICATION_NAME } } }, 
                    // { APPLICATION_NAME: input.APPLICATION_NAME },
                    undefined, 400);
                }
            }
        ...
        if (norm)
            updates.APPLICATION_STATUS = norm;
        const updated = await repo.update(id, updates, auditUser);
        if (!updated) {
