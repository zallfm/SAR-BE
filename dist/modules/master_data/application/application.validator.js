import { systemUsers } from "./mocks"; // atau dari repository master user
function findUser(noreg) {
    return systemUsers.find(u => u.NOREG === noreg);
}
// ➜ aturan baru
function eligibleAsOwner(user) {
    // custodian ⇒ otomatis boleh owner
    return user.canBeOwner || user.canBeCustodian;
}
function eligibleAsCustodian(user) {
...
export function validateOwnerAndCustodian(noregOwner, noregCust) {
