...
export const scheduleService = {
    async getSchedules(app, query) {
        const { page = 1, limit = 10, q, applicationId, applicationName, status, sortBy = "CREATED_DT", order = "desc", } = query;
        ...
        const orderBy = sortBy === "APPLICATION_NAME"
    ...
    async getSchedule(app, key) {
        try {
            ...
            if (!schedule) {
                throw new ApplicationError(ERROR_CODES.APP_NOT_FOUND, ERROR_MESSAGES[ERROR_CODES.APP_NOT_FOUND], 404);
            }
    ...
    async createSchedule(app, data) {
        ...
        try {
            ...
            return formatSchedule(newSchedule); // Cast as any to bypass include
        }
        catch (e) {
            if (e instanceof Prisma.PrismaClientKnownRequestError) {
                if (e.code === "P2002") {
                    throw new ApplicationError(ERROR_CODES.APP_ALREADY_EXISTS, ERROR_MESSAGES[ERROR_CODES.APP_ALREADY_EXISTS], 409);
                }
                if (e.code === "P2003") {
                    throw new ApplicationError(ERROR_CODES.APP_NOT_FOUND, ERROR_MESSAGES[ERROR_CODES.APP_NOT_FOUND], 404);
                }
    ...
    async editSchedule(app, key, data) {
        ...
        try {
            ...
            return formatSchedule(updatedSchedule); // Cast as any to bypass include
        }
        catch (e) {
            if (e instanceof Prisma.PrismaClientKnownRequestError) {
                if (e.code === "P2025") {
                ...
                if (e.code === "P2002") {
                    throw new ApplicationError(ERROR_CODES.APP_ALREADY_EXISTS, "The new combination of Application ID, Sync Start Date, and UAR Date already exists.", 409);
                }
            }
            app.log.error(e);
            throw new ApplicationError(ERROR_CODES.SYS_UNKNOWN_ERROR, ERROR_MESSAGES[ERROR_CODES.SYS_UNKNOWN_ERROR]);
    ...
    async deleteSchedule(app, key) {
        try {
            ...
            return formatSchedule(deletedSchedule); // Cast as any to bypass include
        }
        catch (e) {
            if (e instanceof Prisma.PrismaClientKnownRequestError) {
                if (e.code === "P2025") {
...
export async function runCreateOnlySync(sourcePicList, app) {
    ...
    try {
        ...
        const newTempUarPics = sourcePicList.filter((sourcePic) => !databaseIdMap.has(sourcePic.ID));
        tempUarPic.push(...newTempUarPics);
        app.log.info(`Step 3: Filtered for new records. ${tempUarPic.length} new records found.`);
        const requiredFields = uarPicSchema.body.required;
        let validatedTempPic = tempUarPic.filter((pic) => {
            for (const key of requiredFields) {
                const value = pic[key];
                if (value === null || value === undefined) {
                    app.log.warn(`  Skipping: ${pic.ID} (Reason: Required field ${key} is missing or null).`);
                    return false;
                }
            }
            return true;
        });
        app.log.info(`Step 4: Validated records. ${validatedTempPic.length} records are valid.`);
        if (validatedTempPic.length > 0) {
            ...
            const createResult = await createManyWithManualDuplicateCheck(app, dataToCreate);
            app.log.info(`Step 5: Pushed ${createResult.count} new records to "database".`);
        }
        else {
...
async function createManyWithManualDuplicateCheck(app, dataToCreate) {
    ...
    if (afterInternalDedupCount < originalInputCount) {
        console.log(`Removed ${originalInputCount - afterInternalDedupCount} duplicates from the input data.`);
    }
