 ...
 */
function validateUarPicData(PIC_NAME, DIVISION_ID, // Keep for potential future validation
MAIL) {
    if (!MAIL.endsWith("@toyota.co.id")) {
        throw new ApplicationError(ERROR_CODES.VAL_INVALID_FORMAT, ERROR_MESSAGES[ERROR_CODES.VAL_INVALID_FORMAT], 400);
    }
    if (/\d/.test(PIC_NAME)) {
        throw new ApplicationError(ERROR_CODES.VAL_INVALID_FORMAT, "PIC_NAME should not contain numbers", 400);
    }
}
/**
...
async function dupeCheck(app, MAIL, currentID = null) {
    ...
    if (duplicate) {
        throw new ApplicationError(ERROR_CODES.APP_ALREADY_EXISTS, ERROR_MESSAGES[ERROR_CODES.APP_ALREADY_EXISTS] ||
            "UAR PIC with this MAIL already exists", 400 // 400 (Bad Request) or 409 (Conflict) are appropriate
        );
    }
}
export const uarPicService = {
    ...
    async getUarPics(app, query) {
        const { page = 1, limit = 10, divisionId, q, pic_name, startDate, endDate, sortBy = "CREATED_DT", order = "desc", } = query;
    ...
    async createUarPic(app, PIC_NAME, DIVISION_ID, MAIL, CREATED_BY) {
        ...
        try {
            ...
            if (e instanceof Prisma.PrismaClientKnownRequestError) {
                ...
                if (e.code === "P2002") {
                    app.log.error(e);
                    throw new ApplicationError(ERROR_CODES.APP_ALREADY_EXISTS, `A UAR PIC with this data already exists.`, 400);
                }
            }
            app.log.error(e);
            throw new ApplicationError(ERROR_CODES.SYS_UNKNOWN_ERROR, ERROR_MESSAGES[ERROR_CODES.SYS_UNKNOWN_ERROR]);
    ...
    async editUarPic(app, ID, PIC_NAME, DIVISION_ID, MAIL, CHANGED_BY) {
        ...
        if (!uarPic) {
            throw new ApplicationError(ERROR_CODES.APP_NOT_FOUND, ERROR_MESSAGES[ERROR_CODES.APP_NOT_FOUND], 404 // 404 Not Found is more appropriate
            );
        }
        // --- Run Validations ---
        ...
        try {
            ...
            if (e instanceof Prisma.PrismaClientKnownRequestError) {
                ...
                if (e.code === "P2002") {
                    app.log.error(e);
                    throw new ApplicationError(ERROR_CODES.APP_ALREADY_EXISTS, "UAR PIC with this MAIL already exists", 400);
                }
            }
