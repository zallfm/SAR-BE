 ...
 */
export async function getFunctionEvidence(moduleName, functionName) {
 ...
 */
export async function getModuleEvidence(moduleName) {
    const evidenceFile = path.join(evidenceDir, `${moduleName}.json`);
    if (!existsSync(evidenceFile)) {
        return null;
    }
 ...
 */
export async function exportEvidenceToExcel(moduleName) {
    ...
    for (const moduleEvidence of allEvidence) {
        ...
        for (const funcEvidence of moduleEvidence.functions) {
            ...
            // Coverage Summary
            const overall = (funcEvidence.coverage.statements +
                funcEvidence.coverage.branches +
                funcEvidence.coverage.functions +
                funcEvidence.coverage.lines) / 4;
            wsCoverage.addRow({
                module: moduleEvidence.module,
                function: funcEvidence.functionName,
