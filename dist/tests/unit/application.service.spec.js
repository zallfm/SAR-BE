...
describe("Application Service (simple unit)", () => {
    ...
    it("update validates owner/custodian/security center when provided", async () => {
        ...
        const target = data[0];
        // owner salah
        await expect(svc.update(target.APPLICATION_ID, { NOREG_SYSTEM_OWNER: "00345679" }) // canBeOwner: false
        ).rejects.toMatchObject({ code: "VAL-ERR-302" });
        // custodian salah
        await expect(svc.update(target.APPLICATION_ID, { NOREG_SYSTEM_CUST: "00123457" }) // canBeCustodian: false
        ).rejects.toMatchObject({ code: "VAL-ERR-302" });
        // security center salah
        await expect(svc.update(target.APPLICATION_ID, { SECURITY_CENTER: "SalahSC" })).rejects.toMatchObject({ code: "APP-ERR-103" });
    });
});
