...
describe("uarPicService", () => {
    describe("createUarPic", () => {
        it("should create a new UAR PIC with valid data", async () => {
            ...
            const result = await uarPicService.createUarPic(mockApp, picName, divisionId, mail);
        ...
        it("should throw an invalid format error for an incorrect email domain", async () => {
            ...
            const act = () => uarPicService.createUarPic(mockApp, picName, divisionId, mail);
            await expect(act()).rejects.toThrow(ApplicationError);
            await expect(act()).rejects.toHaveProperty("code", ERROR_CODES.VAL_INVALID_FORMAT);
    ...
    describe("editUarPic", () => {
        it("should edit an existing UAR PIC with valid data", async () => {
            ...
            const result = await uarPicService.editUarPic(mockApp, id, picName, divisionId, mail);
        ...
        it("should throw a not found error for a non-existent ID", async () => {
            ...
            const act = () => uarPicService.editUarPic(mockApp, id, picName, divisionId, mail);
            await expect(act()).rejects.toThrow(ApplicationError);
            await expect(act()).rejects.toHaveProperty("code", ERROR_CODES.APP_NOT_FOUND);
            await expect(act()).rejects.toHaveProperty("statusCode", 400);
        });
